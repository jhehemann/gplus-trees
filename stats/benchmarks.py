...
from stats.stats_gplus_tree import random_klist_tree, random_gtree_of_size
from gplus_trees.gplus_tree_base import gtree_stats_, GPlusTreeBase
from gplus_trees.base import Item


def bench_conversion(num: int, space: int, runs: int = 3) -> None:
    """Benchmark hex conversion of random indices."""
    indices = random.sample(range(space), k=num)
    def _inner():
        for idx in indices:
            _ = idx.to_bytes((space.bit_length() + 7) // 8, 'big').hex()
    t = timeit.timeit(_inner, number=runs) / runs
    print(f"[bench] Hex conversion:            {t:.4f}s for {num} calls")


def bench_rank(num: int, space: int, node_size: int, runs: int = 3) -> None:
    """Benchmark calculate_item_rank on random keys within `space`."""
    indices = random.sample(range(space), k=num)
    key_bytes = (space.bit_length() + 7) // 8
    keys = [i.to_bytes(key_bytes, 'big').hex() for i in indices]
    def _inner():
        for key in keys:
            _ = calculate_item_rank(key, node_size)
    t = timeit.timeit(_inner, number=runs) / runs
...
def bench_build_gtree(sizes: list[int], node_size: int) -> None:
    ...
    for n in sizes:
        ...
        t = random_gtree_of_size(n, node_size)
...
def measure_single_insert(n: int, node_size: int, space: int, trials: int = 200) -> tuple[float, float]:
    """
    Measure per-insert cost into a tree of exactly `n` items,
    averaged over `trials` independent trees.
    Returns (mean_time_s, variance_time_s).
    """
    ...
    key_bytes = (space.bit_length() + 7) // 8
    keys = [random.randrange(space) for _ in range(trials)]
    ranks = [calculate_item_rank(key.to_bytes(key_bytes, 'big').hex(), node_size) for key in keys]
    ...
    return mean(times), variance(times)
...
def bench_single_insert(sizes: list[int], node_size: int, space: int, trials: int) -> None:
    ...
    for n in sizes:
        avg, var = measure_single_insert(n, node_size, space, trials)
        print(
...
def main():
    ...
    parser.add_argument("--num", type=int, default=100_000,
                        help="Number of calls for conversion and rank benchmarks")
    ...
    bench_conversion(args.num, args.space)
